...
def sortString(str):
	return "".join(sorted(str, key=lambda L: (L.lower(), L)))
...
def print_genotype_frequencies(table): # calculates frequencies for each genotype present in table
	...
	for k, x in enumerate(genotypes):
		count = 0
		for y in genotypes:
			if sorted(sortString(x)) == sorted(sortString(y)):
				count += 1
		if sorted(sortString(x)) not in calculated:
			print("The frequency of the " + sortString(x) + " genotype is " + str(float(count)/float((len(genotypes)))*100) + "%.")
